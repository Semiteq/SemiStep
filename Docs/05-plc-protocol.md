...
- [GetChecksum в документации Siemens](https://docs.tia.siemens.cloud/r/en-us/v21/extended-instructions-s7-1200-s7-1500-s7-1200-g2/diagnostics-s7-1200-s7-1500-s7-1200-g2/getchecksum-read-out-checksum-s7-1200-s7-1500-s7-1200-g2)
...
| m8 | PLC_error | WORD | PLC | Код ошибки (см. ниже) |
...
|------|---------|----------|
| Writing (PC_status = 1) | 60 сек | Время на запись всех данных |
| Commit (PC_status = 2) | 30 сек | Время на расчет CRC на PLC |
...
При ошибке checksum или timeout:

- PC повторяет транзакцию с новым `PC_transaction_id`
- Максимум 3 попытки
- После исчерпания попыток — сообщение об ошибке пользователю

> **Примечание:** GetChecksum — ожидается удовлетворительная стоимость подсчёта CRC, можно считать не чаще 1 раза в сек.
...
│  PC                              │  PLC                         │
├──────────────────────────────────┼──────────────────────────────┤
│  Шаг 0: Читает PLC_status        │                              │
│         Если busy → ждёт         │                              │
│         Если idle → продолжить   │                              │
├──────────────────────────────────┼──────────────────────────────┤
│  Шаг 1: PC_status = 1 (writing)  │                              │
│         PC_transaction_id = X    │  Детектирует фронт 0→1       │
│         PC_checksum_* = ...      │  Запускает таймер (60 сек)   │
├──────────────────────────────────┼──────────────────────────────┤
│  Шаг 2: Записывает данные в DB   │                              │
├──────────────────────────────────┼──────────────────────────────┤
│  Шаг 3: PC_status = 2 (commit)   │  Видит переход 1→2           │
│                                  │  PLC_status = 2 (computing)  │
│                                  │  Перезапускает таймер (30с)  │
├──────────────────────────────────┼──────────────────────────────┤
│                                  │  Шаг 4: Считает checksums    │
├──────────────────────────────────┼──────────────────────────────┤
│                                  │  Шаг 5: Сравнивает           │
│                                  │  OK → PLC_status = 3         │
│                                  │  ERR → PLC_status = 4        │
├──────────────────────────────────┼──────────────────────────────┤
│  Шаг 6: Читает PLC_status        │                              │
│         Если success → готово    │                              │
│         Если error → retry (3x)  │                              │
│         PC_status = 0 (idle)     │                              │
├──────────────────────────────────┼──────────────────────────────┤
│                                  │  Шаг 7: Видит PC_status = 0  │
│                                  │  PLC_status = 0 (idle)       │
└──────────────────────────────────┴──────────────────────────────┘
```
...
- Перезапускает таймер (30 секунд на расчёт CRC)
