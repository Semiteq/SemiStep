...
- **Reject on failure.** Preserves the invariant that "the formula always holds in a saved recipe". Bug-safe round-tripping. Cell editing UX: the user's commit is rejected as if `PropertyValidator` had rejected it (existing well-known UX path; the validation panel already surfaces this).
...
- [x] Add nullable `FormulaDefinition? Formula` to `ActionDefinition` (record). **Note: `ActionDefinition.Equals` semantics shift on this field to reference equality** (since `FormulaDefinition` is a class without a custom `Equals`); this is the desired behaviour — two `ActionDefinition`s with separately-constructed `Formula` instances are not considered equal even if their sources match.
...
- Verify rejection path: divide-by-zero and out-of-range failures reject the user's edit, recipe is untouched, validation panel surfaces the structured reason.
