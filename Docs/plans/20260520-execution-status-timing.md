...
- [ ] Переформулировать абзац про прочерк (см. §«Solution Overview ⇒ 5»).
- [ ] Добавить условие формата `d.чч:мм:сс` при ≥ 24 ч.
- [ ] Зафиксировать `(00:00:00, TotalDuration)` для idle-состояния.
