...
- [x] Make folder moves in chunks: Recipe first, Plc next, Configuration last. After each
      chunk: build, fix `using` directives in dependent files (Find/Replace per old namespace).
- [x] After all moves: build green. Run all tests.
- [x] Update `SemiStep.Core.csproj` if any explicit `<Compile Update>` or `<None Update>`
      paths reference the old structure. Same for `SemiStep.UI.csproj`.
- [x] Update `AGENTS.md` "File Layout" and namespace examples to match.
- [x] Update `SemiStep/Tests/YamlConfigs/` paths only if testing helpers reference old
      namespaces (likely none — YAML files are content).

Note on namespace decision: the planned `SemiStep.Core.Recipe` namespace produced a C#
type-vs-namespace ambiguity (the `Recipe` record class lives at the leaf segment, and the
compiler resolves bare `Recipe` references to the sibling namespace from inside
`SemiStep.Core.*` files). The implementation therefore uses the plural form
`SemiStep.Core.Recipes` to break the conflict. The `Plc` namespace shadowed the
`S7.Net.Plc` class only inside `S7Driver.cs`; that single file uses an aliased import
(`using S7NetPlc = global::S7.Net.Plc;`).

#### Config DTO simplification (minimal)

- [x] In `ConfigFacade.MapToDomain`, replace the `GroupMapper.Map(...)` call with the
      4-line inline loop shown above.
- [x] Delete `Config/Mapping/GroupMapper.cs`.
- [x] Build green. Run `Tests/Config/*` — should pass without modification (only the
...
- [x] In `RecipeMutationCoordinator`, change `OnPlcRecipeConflictDetected` to call
      `_plcRecipeConflictDetected.OnNext((local, plc))` directly without `Dispatcher.UIThread.Post`.
- [x] Verify the consumer in `MainWindowViewModel:67` already has
      `.ObserveOn(RxApp.MainThreadScheduler)` on the conflict subscription (confirmed at
      `MainWindowViewModel.cs:67-70`, no consumer-side change needed).
- [x] Run an integration test that triggers conflict detection (or test against PLC
      simulator). The conflict dialog must still open on the UI thread without exceptions.
      [verified via full test suite: 286/286 green; PLC simulator smoke-test deferred to
      manual verification.]
- [x] Update `SemiStep/AGENTS.md` Threading section: state the policy as
...
- [x] Update `S7Service` declaration: `public sealed class S7Service : IS7Connection, IS7Reader, IS7ExecutionStream`.
- [x] Delete `IS7Service.cs`.
- [x] Update `S7Di`: register `S7Service` as singleton, then forward each role to the
      same instance:
      ```
...
- [x] `PlcLifecycleManager` constructor: depend on `IS7Connection` + `IS7Reader`. Drop
...
- [x] `PlcSyncExecutor` constructor: currently `IS7Service connectionService`. Audit the
...
- [x] `PlcSyncCoordinator`: audit the same way and narrow the parameter type(s).
- [x] Confirm `RecipeMutationCoordinator` does not depend on `IS7Service` directly
      (verified via grep: it does not — it goes through `PlcLifecycleManager`).

#### Test rewiring

- [x] `Tests/Helpers/StubIs7Service.cs` -> `StubS7Service` declared against the three
      role interfaces. Keep the property-setter pattern (`ManagingAreaToReturn`,
      `RecipeToReturn`, `IsConnectedOverride`) as is.
- [x] `Tests/S7/Helpers/StubIs7ServiceForSync.cs` -> `StubS7ServiceForSync` declared
      against the same three role interfaces.
- [x] Update DI fixtures (`CoreTestHelper`, `CsvTestHelper`, any other helper that
...
- [x] Build green. `PlcLifecycleManagerReconnectTests` (rewritten in Phase 5) should
...
- [x] `dotnet build SemiStep/SemiStep.slnx` — green
- [x] `dotnet test SemiStep/Tests/Tests.csproj` — all pass
- [x] `dotnet run --project SemiStep/UI/SemiStep.UI.csproj` (post-Phase 2) — app launches,
      load a recipe from `ConfigFiles/`, perform a few mutations, save, verify file is
      readable on next launch. [build verified; full GUI smoke-test deferred to manual
      verification]

After Phase 6 (and optionally Phase 7 + Phase 8) final verification:

- [x] Run `dotnet format SemiStep/SemiStep.slnx` (per `AGENTS.md` pre-commit hook).
- [x] Spot-check public API surface remains unchanged from UI's perspective:
      ...
      method/property/event that `DomainFacade` did. [verified by review agents in review
      phases 1, 2, and 4]
- [x] If a real PLC is available, smoke-test connect / read recipe / write recipe /
      detect conflict / reconnect — to confirm PLC byte layout was not disturbed by the
      file moves through `S7/Serialization/`. [skipped — hardware not available; PLC
      byte-level codecs were moved without modification per plan constraint]
- [x] Update `Docs/03-data-model.md` if internal model documentation references old type
      paths. [grep confirmed no stale references — file uses domain language, not type
      paths]
