...
При закрытии окна с несохранёнными изменениями (крестиком окна или через меню «Файл» → «Выход») появляется диалог подтверждения с кнопками **«Сохранить»**, **«Не сохранять»** и **«Отмена»**. По кнопке «Сохранить» окно закрывается только после успешной записи файла: при ошибке сохранения или отмене диалога выбора файла окно остаётся открытым, а причина отображается в панели сообщений. Закрытие диалога подтверждения (крестиком или клавишей `Esc`) равносильно «Отмене».
...
Интерфейс выполнен в плоском визуальном стиле (в духе JetBrains): без объёмных рамок и градиентов, с единым набором цветов и шрифтов. Язык интерфейса задаётся в конфигурации (`ui/app.yaml`, раздел 6.9); по умолчанию — русский, опционально доступен английский. Локализуется оформление приложения (меню, панель инструментов, подписи статус-бара, диалоги); названия столбцов и действий берутся из конфигурации установки (`ui_name`) и не переводятся.

В каждом окне:

- основная область с табличным представлением рецепта;
- панель инструментов с основными командами (панель действий над строками), включаемая из меню «Вид»;
- статус-бар с индикацией подключения к ПЛК и состояния синхронизации;
- панель сообщений/логов.

![Главное окно с рецептом в состоянии «Подключение…»](./img/app_connection_example.png)
_Главное окно с загруженным рецептом: идёт установление связи с ПЛК («Подключение…»), внизу — панель валидации._
...
#### 2.3.1. Транспонированное представление

Таблица может отображаться и в транспонированном виде: строки соответствуют параметрам,
столбцы — шагам. Переключение — меню «Вид» → «Транспонированная таблица» или `Ctrl+Shift+T`.
Выбор действует в пределах сеанса; при следующем запуске применяется значение `orientation`
из конфигурации (раздел 6.8).

Особенности транспонированного представления:

- клик по ячейке выделяет весь столбец-шаг; повторный клик по уже выделенному столбцу
  открывает редактор ячейки; `Ctrl`/`Shift`+клик добавляет столбец к выделению / расширяет
  диапазон;
- маркер текущего шага исполнения — цветная полоса над столбцом;
- стрелки: вправо/влево — соседний шаг, вверх/вниз — соседний параметр; `Enter` подтверждает
  ввод, `Esc` отменяет его;
- копирование/вставка шагов и файлы CSV сохраняют каноническую раскладку (строки — шаги)
...
| `Ctrl+Shift+T` | Переключение ориентации таблицы (раздел 2.3.1) |
...
![Сообщение об ошибке соединения в панели уведомлений](./img/app_net_error_example.png)
_Сообщение об ошибке соединения с ПЛК в панели уведомлений._
...
| `ui/`         | Оформление таблицы (`grid_style.yaml`) и язык интерфейса (`app.yaml`) | 6.8, 6.9 |
...
Ключ верхнего уровня `orientation` задаёт ориентацию таблицы по умолчанию: `rows_as_steps` (строки — шаги; применяется и при отсутствии ключа) или `columns_as_steps` (транспонированное представление, раздел 2.3.1). Неизвестное значение блокирует запуск с ошибкой конфигурации. Сохранение из окна «Настройки стиля таблицы» всегда записывает действительное значение ключа. Конфигурация RIE поставляется с `columns_as_steps`; MBE и MOCVD ключ не задают.

Значения `grid_style.yaml` (размеры строк и колонок, шрифты, цвета) редактируются как вручную, так и через встроенное окно **«Настройки стиля таблицы»** (меню «Вид» → «Настройки стиля таблицы…»): изменения применяются к таблице и сохраняются в этот файл.

![Окно «Настройки стиля таблицы»](./img/visual_style_window.png)
_Встроенный редактор стиля таблицы: размеры, шрифты и цвета._

---

### 6.9. `app.yaml`

Задаёт язык интерфейса приложения. Файл располагается в каталоге `ui/` рядом с `grid_style.yaml` и **необязателен**: при его отсутствии, отсутствии ключа или недопустимом значении применяется русский язык.

| Ключ     | Обяз. | Значение                            | Пример |
| -------- | ----- | ----------------------------------- | ------ |
| `locale` | нет   | Код языка интерфейса (`ru` \| `en`) | `ru`   |

Локализуется только оформление приложения (меню, панель инструментов, подписи статус-бара, диалоги). Названия столбцов и действий берутся из `ui_name` конфигурации установки и не переводятся; форматы чисел, времени и записи журнала остаются инвариантными независимо от языка.
