...
Состояние синхронизации не содержит отдельного значения для отсутствия связи: наличие соединения с ПЛК отражается отдельной меткой подключения в статус-баре. Если связь теряется при включённой синхронизации, это сообщается как ошибка с текстом «PLC connection lost».
